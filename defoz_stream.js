...
(function () {
  ...
  function proxyLink(link, proxy, proxy_enc, enc) {
    if (link && proxy) {
      return proxy + (proxy.endsWith('/') ? '' : '/') + '?url=' + encodeURIComponent(link);
    }
    return link;
  }
  ...
  function initLang() {
    Lampa.Lang.add({
        ...
        defoz_stream_pro_v2_prefer_http: { ru: 'Предпочитать HTTP (для старых ТВ)' },
        defoz_stream_pro_v2_prefer_mp4: { ru: 'Предпочитать MP4 (вместо HLS)' },
        defoz_stream_pro_v2_collaps_lampa_player: { ru: 'Использовать плеер Lampa для Collaps' },
        defoz_stream_pro_v2_nolink: { ru: 'Ссылка не найдена или заблокирована' },
        defoz_stream_pro_v2_blockedlink: { ru: 'Видео заблокировано правообладателем' }
    });
